...
@Slf4j
public class ReactorHttpClientPostTest extends ReactorBaseTest {
    @ParameterizedTest
    ...
    void testReactorClientServerCombinations(ReactorHttpClientConfig clientConfig, HttpServerConfig serverConfig) throws Exception {
        ...
        client.post(Mono.just(url), Mono.just("hello".getBytes(StandardCharsets.UTF_8))).doOnNext(response -> {
