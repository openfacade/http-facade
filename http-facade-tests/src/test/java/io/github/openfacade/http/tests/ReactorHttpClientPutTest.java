...
@Slf4j
public class ReactorHttpClientPutTest extends ReactorBaseTest {
    @ParameterizedTest
    ...
    void testReactorClientServerCombinations(ReactorHttpClientConfig clientConfig, HttpServerConfig serverConfig) throws Exception {
        ...
        client.put(Mono.just(url), Mono.just("hello".getBytes(StandardCharsets.UTF_8))).doOnNext(response -> {
