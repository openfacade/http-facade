...
@Slf4j
public class ReactorHttpClientPatchTest extends ReactorBaseTest {
    @ParameterizedTest
    ...
    void testReactorClientServerCombinations(ReactorHttpClientConfig clientConfig, HttpServerConfig serverConfig) throws Exception {
        ...
        client.send(HttpMethod.PATCH, Mono.just(url), Mono.just("hello".getBytes(StandardCharsets.UTF_8)), null).doOnNext(response -> {
